...
def test_summarize_action_queues_task(client, monkeypatch, agent, ticket):
    queued = []
    ...
    monkeypatch.setattr("apps.tickets.views.summarize_thread", FakeTask)
    client.force_login(agent)

    response = client.post(reverse("tickets:summarize", args=[ticket.id]))
